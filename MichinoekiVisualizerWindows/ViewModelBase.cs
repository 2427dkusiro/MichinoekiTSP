...
public abstract class ViewModelBase : INotifyPropertyChanged, INotifyDataErrorInfo
{
    ...
    /// </summary>
    private readonly Dictionary<string, List<DelegateCommand>> commandToPropertyDependencyDic = new();
    ...
    {
        return errors.TryGetValue(propertyName, out List<string>? v) && v.Count > 0;
    }
    ...
    protected DelegateCommand RegisterCommand(DelegateCommand delegateCommand, params string[] propertyNames)
    {
        ...
        {
            if (commandToPropertyDependencyDic.TryGetValue(prop, out List<DelegateCommand>? value))
            {
                value.Add(delegateCommand);
            }
            else
            {
